class InventoriesController < ApplicationController
  ...
  before_action :find_invent, only: [:update,:destroy]
  # GET /inventories
  ...
  def show
@inventory = @venue.inventories.find(params[:vd])  
  ...
  private
    ...
    def find_invent
    @inventory = Inventory.find(params[:vd])
    end


    # Never trust parameters from the scary internet, only allow the white list through.
    def inventory_params
      params.require(:inventory).permit(:name, :price,:image, :category)
    end
end
